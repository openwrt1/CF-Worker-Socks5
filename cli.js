...
const ipInCFCidr = (ip) => {
  if (ip.indexOf(":") == -1) {
    ...
    return {
      cf: ADDR4.some(
        ({ a, m }) =>
          (ipa.reduce((s, b, i) => (s += b << (24 - 8 * i)), 0) & m) ===
          (a & m),
      ),
      ip: ip,
    };
  ...
  new Promise((res, rej) => {
    if (CFDOMAN.includes(host)) res({ cf: true, ip: host });
      ...
      if (ATYP == 0x01 || ATYP == 0x04) res((cache[host] = ipInCFCidr(host)));
        ...
        dns(host)
          .then((ip) => res((cache[host] = ipInCFCidr(ip))))
              ...
              isCFIP(host, ATYP).then(({ cf, ip }) => {
                new Promise((res, rej) => {
                  if (cf && !proxyip) {
                    ...
                    net
                      .connect(connectPort, wkip ? wkip : ip, function () {
                        res(this);
                      })
                      .on("error", rej);
                  } else {
                    ...
                    new WebSocket(url, opts)
                      .on("open", function (e) {
                        this.send(
                          JSON.stringify({
                            hostname: cf ? proxyip : ip,
                            port: cf ? proxyport : port,
                            psw,
                          }),
                        );
